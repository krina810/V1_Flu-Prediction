...
// connect with mysqli
$con = mysqli_connect("localhost", "root", "", "flu");
// Check connection
...
$result = mysqli_query($con,"SELECT cache_url FROM flu_engine");
...
do {
  ...
  include('oauth.php');
  $result = mysqli_query($con,"SELECT bearer_token FROM flu_engine");
  $field = mysqli_fetch_array($result, MYSQLI_ASSOC);
    ...
    foreach ($json['statuses'] as $tweet) 
    {
      ...
      //echo "<br />**********************************************<br />**********************************************<br />";
        ...
        if ( isset($geo_tweet_location_lat) OR isset($result) ) {
          $stripped_tweet = mysqli_real_escape_string($con,$stripped_tweet);
          $unix_time = strtotime($datetime);
          
          if (isset($geo_tweet_location_lat)){
            mysqli_query($con,"INSERT INTO flu_tweets (user_name, user_id, tweet_text, tweet_id, tweet_date, latitude, longitude, aggravation )
...
if (!isset($we_have_errors)){
  ...
  mysqli_query($con,"UPDATE flu_engine SET cache_url='$cache'"); //SAVE the refresh url
}
